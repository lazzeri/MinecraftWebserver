...
import {
  ...
  a1 as descriptors,
  a2 as isFunction,
  a3 as _attachContext,
  a4 as _createResolver,
  a5 as _descriptors,
  a6 as mergeIf,
  a7 as uid,
  a8 as debounce,
  ...
  aa as clearCanvas,
  ab as setsEqual,
  ac as _elementsEqual,
  ad as getAngleFromPoint,
  ae as _readValueToProps,
  af as _updateBezierControlPoints,
  ag as _computeSegments,
  ah as _boundSegments,
  ai as _steppedInterpolation,
  aj as _bezierInterpolation,
  ak as _pointInLine,
  al as _steppedLineTo,
  am as _bezierCurveTo,
  an as drawPoint,
  ao as addRoundedRectPath,
  ap as toTRBL,
  aq as toTRBLCorners,
  ar as _boundSegment,
  as as _normalizeAngle,
  at as getRtlAdapter,
  au as overrideTextDirection,
  av as _textX,
  aw as restoreTextDirection,
  ax as noop,
  ay as distanceBetweenPoints,
  az as _setMinAndMaxByKey,
  aA as niceNum,
  aB as almostWhole,
  aC as almostEquals,
  aD as _decimalPlaces,
  aE as _longestText,
  aF as _filterBetween,
  aG as _lookup,
...
function clearStacks(meta, items) {
  ...
  for (const parsed of items) {
    ...
    if (
      !stacks ||
      stacks[meta.vScale.id] === undefined ||
      stacks[meta.vScale.id][meta.index] === undefined
    ) {
      return;
    }
...
class DatasetController {
  ...
  getMinMax(scale, canStack) {
    ...
    const stack = canStack &&
      meta._stacked && { keys: getSortedDatasetIndices(me.chart, true), values: null };
  ...
  _update(mode) {
    ...
    meta._clip = toClip(
      valueOrDefault(me.options.clip, defaultClip(meta.xScale, meta.yScale, me.getMaxOverflow())),
    );
  }
  update(mode) {}
  draw() {
  ...
  getContext(index, active, mode) {
    ...
    if (index >= 0 && index < me._cachedMeta.data.length) {
      ...
      context =
        element.$context || (element.$context = createDataContext(me.getContext(), index, element));
      context.parsed = me.getParsed(index);
      context.raw = dataset.data[index];
      context.index = context.dataIndex = index;
    } else {
      context =
        me.$context || (me.$context = createDatasetContext(me.chart.getContext(), me.index));
      ...
      const vPixel = (properties[vAxis] =
        reset || nullData
          ...
          : vScale.getPixelForValue(
              _stacked ? me.applyStack(vScale, parsed, _stacked) : parsed[vAxis],
              i,
            ));
  ...
  for (i = 0, ilen = boxes.length; i < ilen; ++i) {
    ...
    if (layout.horizontal) {
      ...
      box.bottom = box.fullSize
        ? params.outerHeight - userPadding.right
  ...
  _calculatePadding(first, last, sin, cos) {
    ...
    if (me.isHorizontal()) {
      ...
      me.paddingLeft = Math.max(
        ((paddingLeft - offsetLeft + padding) * me.width) / (me.width - offsetLeft),
        0,
      );
      me.paddingRight = Math.max(
        ((paddingRight - offsetRight + padding) * me.width) / (me.width - offsetRight),
        0,
      );
    } else {
...
const itemsEqual = (a, b) =>
  a !== null && b !== null && a.datasetIndex === b.datasetIndex && a.index === b.index;
    ...
    if (this.isHorizontal()) {
      ...
      y =
        ...
        _alignStartEnd(
          opts.align,
          me.top,
          me.bottom - maxHeight - opts.labels.padding - me._computeTitleHeight(),
        );
    }
...
function generateTicks$1(generationOptions, dataRange) {
  ...
  const { step, min, max, precision, count, maxTicks, maxDigits, includeBounds } =
    generationOptions;
  const unit = step || 1;
...
class TimeScale extends Scale {
  ...
  _generate() {
    ...
    const minor =
      timeOpts.unit ||
      determineUnitForAutoTicks(timeOpts.minUnit, min, max, me._getLabelCapacity(min));
