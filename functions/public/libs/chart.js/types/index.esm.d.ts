...
export interface Plugin<TType extends ChartType = ChartType, O = AnyObject>
  ...
  beforeEvent?(
    chart: Chart,
    args: { event: ChartEvent; replay: boolean; cancelable: true },
    options: O,
  ): boolean | void;
  /**
  ...
  afterEvent?(
    chart: Chart,
    args: { event: ChartEvent; replay: boolean; changed?: boolean; cancelable: false },
    options: O,
  ): void;
  /**
...
export interface BarElement<T extends BarProps = BarProps, O extends BarOptions = BarOptions>
  extends Element<T, O>,
    VisualElement {}
...
export interface TooltipOptions<TType extends ChartType> extends CoreInteractionOptions {
  ...
  filter: (
    e: TooltipItem<ChartType>,
    index: number,
    array: TooltipItem<ChartType>[],
    data: ChartData,
  ) => boolean;
...
export type LogarithmicScale<O extends LogarithmicScaleOptions = LogarithmicScaleOptions> =
  Scale<O>;
...
export interface RadialLinearScale<O extends RadialLinearScaleOptions = RadialLinearScaleOptions>
  extends Scale<O> {
  setCenterPoint(
    leftMovement: number,
