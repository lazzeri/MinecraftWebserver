...
RenderDom.init = function (Matter) {
  ...
  RenderDom.bodies = function (render, bodies, context) {
    ...
    var c = context,
      ...
      matterBodies = Composite.allBodies(overlayObj.world),
      domBody;
    ...
    for (var i = 0; i < matterBodies.length; i++) {
      ...
      for (var k = matterBody.parts.length > 1 ? 1 : 0; k < matterBody.parts.length; k++) {
        ...
        var domPart = $('#' + matterPart.Dom.element).get(0);
        ...
        if (bodyWorldPoint.y > 2000) {
          overlayObj.World.remove(overlayObj.world, matterPart);
          $(domPart).remove();
        }
      }
    }
