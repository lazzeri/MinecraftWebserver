...
gridstack_1.GridStack.prototype._setupAcceptWidget = function () {
  ...
  GridStackDD.get()
    .droppable(this.el, {
      accept: (el) => {
        ...
        if (typeof this.opts.acceptWidgets === 'function') {
          ...
          let selector =
            this.opts.acceptWidgets === true ? '.grid-stack-item' : this.opts.acceptWidgets;
    ...
    .on(this.el, 'dropover', (event, el, helper) => {
      ...
      if (
        (node === null || node === void 0 ? void 0 : node.grid) === this &&
      ...
      if (
        (node === null || node === void 0 ? void 0 : node.grid) &&
        node.grid !== this &&
        !node._temporaryRemoved
      ) {
      ...
      if (node.grid && node.grid !== this) {
        ...
        node._initDD =
          ...
          node._temporaryRemoved =
            true; // so it can be inserted onDrag below
      } else {
        node.w = w;
        node.h = h;
    ...
    .on(this.el, 'drop', (event, el, helper) => {
      ...
      if (this._gsEventHandler['dropped']) {
        this._gsEventHandler['dropped'](
          Object.assign(Object.assign({}, event), { type: 'dropped' }),
...
gridstack_1.GridStack.prototype._setupRemoveDrop = function () {
  if (!this.opts.staticGrid && typeof this.opts.removable === 'string') {
    ...
    if (!GridStackDD.get().isDroppable(trashEl)) {
      GridStackDD.get()
        .droppable(trashEl, this.opts.removableOptions)
...
gridstack_1.GridStack.prototype._onStartMoving = function (
  el,
  event,
  ui,
  node,
...
gridstack_1.GridStack.prototype._dragOrResize = function (
  el,
  event,
  ui,
  node,
  ...
  if (
    this.engine.moveNodeCheck(
      node,
      Object.assign(Object.assign({}, p), { cellWidth, cellHeight, rect, resizing }),
    )
  ) {
    node._lastUiPosition = ui.position;
...
gridstack_1.GridStack.prototype.movable = function (els, val) {
  ...
  gridstack_1.GridStack.getElements(els).forEach((el) => {
    let node = el.gridstackNode;
    if (!node) return;
    if (val) delete node.noMove;
    else node.noMove = true;
...
gridstack_1.GridStack.prototype.resizable = function (els, val) {
  ...
  gridstack_1.GridStack.getElements(els).forEach((el) => {
    let node = el.gridstackNode;
    if (!node) return;
    if (val) delete node.noResize;
    else node.noResize = true;
    this._prepareDragDropByNode(node); // init DD if need be, and adjust
  });
  return this;
};
