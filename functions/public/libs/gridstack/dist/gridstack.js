...
class GridStack {
  ...
  constructor(el, opts = {}) {
    ...
    let defaults = Object.assign(Object.assign({}, utils_1.Utils.cloneDeep(GridDefaults)), {
  ...
  load(layout, addAndRemove = true) {
    ...
    if (
      this._prevColumn &&
      this._prevColumn !== this.opts.column &&
      items.some((n) => n.x + n.w > this.opts.column)
    ) {
  ...
  getGridItems() {
    return Array.from(this.el.children).filter(
      (el) =>
        el.matches('.' + this.opts.itemClass) && !el.matches('.' + this.opts.placeholderClass),
    );
  }
  /**
    ...
    GridStack.getElements(els).forEach((el) => {
      ...
      for (const key in w) {
        if (key[0] !== '_' && n[key] !== w[key]) {
          ...
          ddChanged =
            ddChanged ||
            (!this.opts.staticGrid && (key === 'noResize' || key === 'noMove' || key === 'locked'));
        }
      }
    ...
    if (this._isAutoCellHeight) {
      if (!changedOneColumn && this.opts.cellHeightThrottle) {
        if (!this._cellHeightThrottle) {
          this._cellHeightThrottle = utils_1.Utils.throttle(
            () => this.cellHeight(),
