...
// },
const $ = require('functions/public/libs/gridstack/dist/jq/jquery'); // compile this in... having issues TS/ES6 app would include instead
exports.$ = $;
require('./jquery-ui');
...
class GridStackDDJQueryUI extends gridstack_dd_1.GridStackDD {
  resizable(el, opts, key, value) {
    ...
    if (opts === 'enable') {
      ...
      let handles = $el.data('gs-resize-handles')
        ? $el.data('gs-resize-handles')
        : grid.opts.resizable.handles;
      $el.resizable(
        Object.assign(Object.assign(Object.assign({}, grid.opts.resizable), { handles: handles }), {
          start: opts.start,
          stop: opts.stop,
  ...
  draggable(el, opts, key, value) {
    ...
    if (opts === 'enable') {
      ...
      const grid = el.gridstackNode.grid;
      $el.draggable(
        Object.assign(Object.assign({}, grid.opts.draggable), {
          containment:
            grid.opts._isNested && !grid.opts.dragOut
              ? $(grid.el).parent()
  ...
  on(el, name, callback) {
    ...
    $el.on(name, (event, ui) => {
      return callback(
        event,
        ui.draggable ? ui.draggable[0] : event.target,
        ui.helper ? ui.helper[0] : null,
      );
    });
    return this;
  }
