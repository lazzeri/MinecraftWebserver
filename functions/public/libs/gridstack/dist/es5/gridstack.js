...
var GridStack = /** @class */ (function () {
  ...
  function GridStack(el, opts) {
    ...
    var defaults = __assign(__assign({}, utils_1.Utils.cloneDeep(GridDefaults)), {
    ...
    if (this._isAutoCellHeight || this.opts.cellHeight === 'initial') {
      ...
      if (
        typeof this.opts.cellHeight == 'number' &&
        this.opts.cellHeightUnit &&
        this.opts.cellHeightUnit !== GridDefaults.cellHeightUnit
      ) {
  ...
  GridStack.prototype.load = function (layout, addAndRemove) {
    var _this = this;
    if (addAndRemove === void 0) {
      addAndRemove = true;
    }
    var items = GridStack.Utils.sort(
      __spreadArrays(layout),
      -1,
      this._prevColumn || this.opts.column,
    ); // make copy before we mod/sort
    ...
    if (
      this._prevColumn &&
      this._prevColumn !== this.opts.column &&
      items.some(function (n) {
        return n.x + n.w > _this.opts.column;
      })
    ) {
    ...
    items.forEach(function (w) {
      var item =
        w.id || w.id === 0
          ? _this.engine.nodes.find(function (n) {
              return n.id === w.id;
            })
          : undefined;
      if (item) {
  ...
  GridStack.prototype.getGridItems = function () {
    ...
    return Array.from(this.el.children).filter(function (el) {
      return (
        el.matches('.' + _this.opts.itemClass) && !el.matches('.' + _this.opts.placeholderClass)
      );
    });
  };
  /**
  ...
  GridStack.prototype.removeWidget = function (els, removeDOM, triggerEvent) {
    ...
    GridStack.getElements(els).forEach(function (el) {
      if (el.parentElement !== _this.el) return; // not our child!
  ...
  GridStack.prototype.setStatic = function (val, updateClass) {
    ...
    this._setupAcceptWidget();
    this.engine.nodes.forEach(function (n) {
      return _this._prepareDragDropByNode(n);
    ...
    GridStack.getElements(els).forEach(function (el) {
      ...
      for (var key in w) {
        if (key[0] !== '_' && n[key] !== w[key]) {
          ...
          ddChanged =
            ddChanged ||
            (!_this.opts.staticGrid &&
              (key === 'noResize' || key === 'noMove' || key === 'locked'));
        }
      }
      // finally move the widget
      if (m) {
        _this.engine.cleanNodes().beginUpdate(n).moveNode(n, m);
  ...
  GridStack.prototype.willItFit = function (node) {
    ...
    if (arguments.length > 1) {
      ...
      var a = arguments,
        i = 0,
        w = { x: a[i++], y: a[i++], w: a[i++], h: a[i++], autoPosition: a[i++] };
  ...
  GridStack.prototype._updateStyles = function (forceUpdate, maxH) {
    ...
    if (!this._styles) {
      ...
      var placeholder =
        '.' + this.opts._styleSheetClass + ' > .grid-stack-placeholder > .placeholder-content';
      utils_1.Utils.addCSSRule(
        this._styles,
        content,
        'top: ' + top_1 + '; right: ' + right + '; bottom: ' + bottom + '; left: ' + left + ';',
      );
      utils_1.Utils.addCSSRule(
        this._styles,
        placeholder,
