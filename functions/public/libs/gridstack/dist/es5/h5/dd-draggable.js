...
var DDDraggable = /** @class */ (function (_super) {
  ...
  DDDraggable.prototype.updateOption = function (opts) {
    ...
    Object.keys(opts).forEach(function (key) {
      return (_this.option[key] = opts[key]);
    });
    return this;
  };
  /** @internal */
  ...
  DDDraggable.prototype._createHelper = function (event) {
    ...
    if (!document.body.contains(helper)) {
      dd_utils_1.DDUtils.appendTo(
        helper,
        this.option.appendTo === 'parent' ? this.el.parentNode : this.option.appendTo,
      );
    }
    if (helper === this.el) {
      this.dragElementOriginStyle = DDDraggable.originStyleProp.map(function (prop) {
        return _this.el.style[prop];
      });
    }
    return helper;
  };
  ...
  DDDraggable.prototype._dragFollow = function (event) {
    ...
    this.paintTimer = requestAnimationFrame(function () {
      ...
      if (_this.helper.style.position === 'absolute') {
        var _a = _this.helperContainment.getBoundingClientRect(),
          left = _a.left,
          top_1 = _a.top;
